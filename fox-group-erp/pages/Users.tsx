...
const Users: React.FC = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const usersData = (response.data as any)?.results || response.data;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', username: '', password: '123456', role: 'cashier' });
        ...
        {users.map(user => (
          <div key={user.id} className="bg-dark-950 border border-dark-800 rounded-xl p-6 relative group">
            <div className="flex items-center gap-4 mb-4">
              <div className={`w-14 h-14 rounded-full flex items-center justify-center text-xl font-bold ${user.role === 'admin' ? 'bg-red-500/20 text-red-500' :
              ...
              <div>
                ...
                <span className={`px-2 py-0.5 rounded text-xs border ${user.role === 'admin' ? 'bg-red-500/10 text-red-400 border-red-500/20' :
