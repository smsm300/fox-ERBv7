...
import { productsAPI, transactionsAPI } from '../services/endpoints';
...
const Inventory: React.FC<InventoryProps> = ({ products: initialProducts, transactions: initialTransactions, onAddProduct, onUpdateProduct, onDeleteProduct, onStockAdjustment, settings }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingProduct) {
        ...
        setProducts(prev => prev.map(p => p.id === updated.data.id ? updated.data : p));
        onUpdateProduct(updated.data);
      } else {
        const created = await productsAPI.create(formData as Omit<Product, 'id'>);
        setProducts(prev => [...prev, created.data]);
        onAddProduct(created.data);
      }
      setIsFormOpen(false);
    } catch (err: any) {
      setError(handleAPIError(err));
  ...
  const handleAdjustmentSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!adjustmentForm.productId || adjustmentForm.quantityDiff === 0) return;
    
    setLoading(true);
    setError('');
    try {
      await productsAPI.adjustStock(adjustmentForm.productId, {
        quantity_diff: adjustmentForm.quantityDiff,
