...
import { customersAPI, transactionsAPI } from '../services/endpoints';
...
const Customers: React.FC<CustomersProps> = ({ customers: initialCustomers, transactions: initialTransactions, onAddCustomer, onUpdateCustomer, onDeleteCustomer, settings }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCustomer) {
        ...
        setCustomers(prev => prev.map(c => c.id === updated.data.id ? updated.data : c));
        onUpdateCustomer(updated.data);
      } else {
        const created = await customersAPI.create(formData);
        setCustomers(prev => [...prev, created.data]);
        onAddCustomer(created.data);
      }
  ...
  const handleSettleDebt = async (customerId: number, amount: number, notes: string) => {
    setLoading(true);
    setError('');
    try {
      await customersAPI.settleDebt(customerId, { amount, payment_method: 'cash' as any });
