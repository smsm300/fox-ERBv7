...
const Suppliers: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingSupplier) {
        const updated = await suppliersAPI.update(editingSupplier.id, formData);
  ...
  const handleSettleDebt = async (supplierId: number, amount: number, notes: string) => {
    setLoading(true);
    setError('');
    try {
      await suppliersAPI.settleDebt(supplierId, { amount, payment_method: 'cash' as any });
      await fetchSuppliers();
